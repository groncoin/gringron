...
use crate::extkey_bip32::{BIP32GrinGronHasher, ExtendedPrivKey, ExtendedPubKey};
use crate::types::{
...
impl ExtKeychain {
	...
	pub fn hasher(&self) -> BIP32GrinGronHasher {
		self.hasher.clone()
	}
}
...
impl Keychain for ExtKeychain {
	fn from_seed(seed: &[u8], is_test: bool) -> Result<ExtKeychain, Error> {
		let mut h = BIP32GrinGronHasher::new(is_test);
	...
	fn from_mnemonic(word_list: &str, extension_word: &str, is_test: bool) -> Result<Self, Error> {
		let secp = secp::Secp256k1::with_caps(secp::ContextFlag::Commit);
		let h = BIP32GrinGronHasher::new(is_test);
