...
//! Modified from above to integrate into gringron and allow for different
...
/// GrinGron uses blake2 everywhere but the spec calls for SHA512/Ripemd160, so allow
...
#[derive(Clone, Debug)]
pub struct BIP32GrinGronHasher {
	is_test: bool,
	hmac_sha512: Hmac<Sha512>,
}

impl BIP32GrinGronHasher {
	/// New empty hasher
	pub fn new(is_test: bool) -> BIP32GrinGronHasher {
		BIP32GrinGronHasher {
			is_test: is_test,
...
impl BIP32Hasher for BIP32GrinGronHasher {
	fn network_priv(&self) -> [u8; 4] {
		if self.is_test {
			[0x03, 0x27, 0x3A, 0x10]
		} else {
...
impl ExtendedPrivKey {
	...
	pub fn from_mnemonic(
		...
		let mut hasher = BIP32GrinGronHasher::new(is_test);
