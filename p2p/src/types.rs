...
impl From<gringron_store::Error> for Error {
	fn from(e: gringron_store::Error) -> Error {
		Error::Store(e)
	}
}
...
impl PeerAddr {
	...
	/// defaults to port 5634 on mainnet and 15634 on testnet.
	pub fn from_ip(addr: IpAddr) -> PeerAddr {
		let port = if global::is_testnet() { 15634 } else { 5634 };
...
/// included in gringron-server.toml, but we don't want them to ever return none
impl P2PConfig {
	/// return ban window
...
pub trait ChainAdapter: Sync + Send {
	...
	/// Get the GrinGron specific tmp dir
