...
use crate::auth::{
	BasicAuthMiddleware, BasicAuthURIMiddleware, GRINGRON_BASIC_REALM, GRINGRON_FOREIGN_BASIC_REALM,
};
use crate::chain;
	...
	P: PoolAdapter + 'static,
{
	...
	if let Some(api_secret) = api_secret {
		let api_basic_auth =
			"Basic ".to_string() + &to_base64(&("gringron:".to_string() + &api_secret));
		let basic_auth_middleware = Arc::new(BasicAuthMiddleware::new(
			api_basic_auth,
			&GRINGRON_BASIC_REALM,
