...
/// * When running `gringron` with defaults, the V2 api is available at
/// `localhost:5633/v2/owner`
...
pub trait OwnerRpc: Sync + Send {
	...
	```
	# gringron_api::doctest_helper_json_rpc_owner_assert_response!(
	# r#"
	{
		"jsonrpc": "2.0",
		"method": "get_status",
...
macro_rules! doctest_helper_json_rpc_owner_assert_response {
	($request:expr, $expected_response:expr) => {
		// create temporary gringron server, run jsonrpc request on node api, delete server, return
		// json response.

		{
			/*use gringron_servers::test_framework::framework::run_doctest;
