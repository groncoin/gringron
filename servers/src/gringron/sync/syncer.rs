...
use crate::gringron::sync::body_sync::BodySync;
use crate::gringron::sync::header_sync::HeaderSync;
use crate::gringron::sync::state_sync::StateSync;
use crate::p2p;
...
impl SyncRunner {
	...
	fn wait_for_min_peers(&self) -> Result<(), chain::Error> {
		...
		loop {
			...
			// * we are synced already, e.g. gringron was quickly restarted
			// * timeout
			if wp > MIN_PEERS
				|| (wp == 0
