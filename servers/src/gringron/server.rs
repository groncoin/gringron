...
//! GrinGron server implementation, glues the different parts of the system (mostly
...
use crate::gringron::{dandelion_monitor, seed, sync};
...
use gringron_util::logger::LogEntry;
...
/// GrinGron server holding internal structures.
pub struct Server {
	/// server config
	...
	/// Maintain a lock_file so we do not run multiple GrinGron nodes from same dir.
	lock_file: Arc<File>,
...
impl Server {
	...
	// instance of gringron server from the same dir.
	// This uses fs2 and should be safe cross-platform unless somebody abuses the file itself.
	fn one_gringron_at_a_time(config: &ServerConfig) -> Result<Arc<File>, Error> {
		let path = Path::new(&config.db_root);
		fs::create_dir_all(&path)?;
		let path = path.join("gringron.lock");
		...
		lock_file.try_lock_exclusive().map_err(|e| {
			...
			writeln!(
				&mut stderr,
				"Failed to lock {:?} (gringron server already running?)",
				path
			)
	...
	pub fn new(
		...
		let lock_file = Server::one_gringron_at_a_time(&config)?;
	...
	pub fn start_test_miner(
		...
		let config_wallet_url = match wallet_listener_url.clone() {
			Some(u) => u,
			None => String::from("http://127.0.0.1:15635"),
		};
