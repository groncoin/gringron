...
impl Default for ServerConfig {
	fn default() -> ServerConfig {
		ServerConfig {
			db_root: "gringron_chain".to_string(),
			api_http_addr: "127.0.0.1:5633".to_string(),
...
impl Default for StratumServerConfig {
	fn default() -> StratumServerConfig {
		StratumServerConfig {
			wallet_listener_url: "http://127.0.0.1:5635".to_string(),
			burn_reward: false,
			attempt_time_per_block: 15,
			minimum_share_difficulty: 1,
			enable_stratum_server: Some(false),
			stratum_server_addr: Some("127.0.0.1:5636".to_string()),
		}
	}
}
