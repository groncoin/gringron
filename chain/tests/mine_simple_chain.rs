...
use gringron_chain::{BlockStatus, ChainAdapter, Options};
...
fn test_block_a_block_b_block_b_fork_header_c_fork_block_c() {
	let chain_dir = ".gringron.block_a_block_b_block_b_fork_header_c_fork_block_c";
	clean_output_dir(chain_dir);
...
fn test_block_a_block_b_block_b_fork_header_c_fork_block_c_fork() {
	let chain_dir = ".gringron.block_a_block_b_block_b_fork_header_c_fork_block_c_fork";
	clean_output_dir(chain_dir);
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	let genesis = pow::mine_genesis_block().unwrap();
...
fn test_block_a_header_b_header_b_fork_block_b_fork_block_b_block_c() {
	let chain_dir = ".gringron.test_block_a_header_b_header_b_fork_block_b_fork_block_b_block_c";
	clean_output_dir(chain_dir);
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	let genesis = pow::mine_genesis_block().unwrap();
...
fn test_block_a_header_b_header_b_fork_block_b_fork_block_b_block_c_fork() {
	let chain_dir = ".gringron.test_block_a_header_b_header_b_fork_block_b_fork_block_b_block_c_fork";
	clean_output_dir(chain_dir);
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	let genesis = pow::mine_genesis_block().unwrap();
...
fn mine_reorg() {
	...
	const DIR_NAME: &str = ".gringron_reorg";
	clean_output_dir(DIR_NAME);
...
#[test]
fn mine_forks() {
	clean_output_dir(".gringron2");
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	{
		let chain = init_chain(".gringron2", pow::mine_genesis_block().unwrap());
		let kc = ExtKeychain::from_random_seed(false).unwrap();
...
#[test]
fn mine_losing_fork() {
	clean_output_dir(".gringron3");
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	{
		let chain = init_chain(".gringron3", pow::mine_genesis_block().unwrap());
...
fn longer_fork() {
	...
	let genesis = pow::mine_genesis_block().unwrap();
	{
		let chain = init_chain(".gringron4", genesis.clone());
...
fn spend_rewind_spend() {
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	let chain_dir = ".gringron_spend_rewind_spend";
	clean_output_dir(chain_dir);
...
fn spend_in_fork_and_compact() {
	clean_output_dir(".gringron6");
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	{
		let chain = init_chain(".gringron6", pow::mine_genesis_block().unwrap());
		let prev = chain.head_header().unwrap();
		let kc = ExtKeychain::from_random_seed(false).unwrap();
		let pb = ProofBuilder::new(&kc);
...
fn output_header_mappings() {
	clean_output_dir(".gringron_header_for_output");
	global::set_local_chain_type(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	{
		clean_output_dir(".gringron_header_for_output");
		let chain = init_chain(
			".gringron_header_for_output",
...
fn test_overflow_cached_rangeproof() {
	clean_output_dir(".gringron_overflow");
	global::set_local_chain_type(ChainTypes::AutomatedTesting);

	util::init_test_logger();
	{
		let chain = init_chain(".gringron_overflow", pow::mine_genesis_block().unwrap());
		let prev = chain.head_header().unwrap();
		let kc = ExtKeychain::from_random_seed(false).unwrap();
		let pb = ProofBuilder::new(&kc);
		...
		// and a positive output for 1m gringron
