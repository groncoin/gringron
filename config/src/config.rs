...
pub const SERVER_CONFIG_FILE_NAME: &str = "gringron-server.toml";
const SERVER_LOG_FILE_NAME: &str = "gringron-server.log";
const GRINGRON_HOME: &str = ".gringron";
const GRINGRON_CHAIN_DIR: &str = "chain_data";
...
fn get_gringron_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
...
fn check_api_secret_files(
	...
	let gringron_path = get_gringron_path(chain_type)?;
...
impl GlobalConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Testnet => {
				defaults.api_http_addr = "127.0.0.1:15633".to_owned();
				defaults.p2p_config.port = 15634;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:15636".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:15635".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_http_addr = "127.0.0.1:25633".to_owned();
				defaults.p2p_config.port = 25634;
				defaults.p2p_config.seeding_type = p2p::Seeding::None;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:25636".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:25635".to_owned();
			}
	...
	/// Update paths
	pub fn update_paths(&mut self, gringron_home: &PathBuf) {
		// need to update server chain path
		let mut chain_path = gringron_home.clone();
		chain_path.push(GRINGRON_CHAIN_DIR);
		self.members.as_mut().unwrap().server.db_root = chain_path.to_str().unwrap().to_owned();
		let mut api_secret_path = gringron_home.clone();
		...
		let mut foreign_api_secret_path = gringron_home.clone();
		...
		let mut log_path = gringron_home.clone();
