...
use crate::consensus::GRINGRON_BASE;
...
pub fn amount_from_hr_string(amount: &str) -> Result<u64, Error> {
	...
	let (gringrons, ngringrons) = match amount.find('.') {
		None => (parse_gringrons(amount)?, 0),
		Some(pos) => {
			let (gs, tail) = amount.split_at(pos);
			(parse_gringrons(gs)?, parse_ngringrons(&tail[1..])?)
		}
	};
	Ok(gringrons * GRINGRON_BASE + ngringrons)
}

fn parse_gringrons(amount: &str) -> Result<u64, Error> {
	if amount == "" {
		Ok(0)
	} else {
		amount
...
lazy_static! {
	static ref WIDTH: usize = (GRINGRON_BASE as f64).log(10.0) as usize + 1;
}

fn parse_ngringrons(amount: &str) -> Result<u64, Error> {
...
pub fn amount_to_hr_string(amount: u64, truncate: bool) -> String {
	let amount = (amount as f64 / GRINGRON_BASE as f64) as f64;
