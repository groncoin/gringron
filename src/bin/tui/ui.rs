...
impl UI {
	...
	pub fn new(
		...
		//set theme
		let mut theme = gringron_ui.cursive.current_theme().clone();
		modify_theme(&mut theme);
		gringron_ui.cursive.set_theme(theme);
		gringron_ui.cursive.add_fullscreen_layer(main_layer);

		// Configure a callback (shutdown, for the first test)
		let controller_tx_clone = gringron_ui.controller_tx.clone();
		gringron_ui.cursive.add_global_callback('q', move |c| {
