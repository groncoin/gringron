...
/// GrinGron client commands processing
...
impl HTTPNodeClient {
	...
	pub fn show_status(&self) {
		println!();
		let title = "GrinGron Server Status".to_string();
		...
		match self.send_json_request::<Status>("get_status", &serde_json::Value::Null) {
			...
			Err(_) => writeln!(
				e,
				"WARNING: Client failed to get data. Is your `gringron server` offline or broken?"
			)
			.unwrap(),
		};
		e.reset().unwrap();
...
pub fn client_command(client_args: &ArgMatches<'_>, global_config: GlobalConfig) -> i32 {
	...
	match client_args.subcommand() {
		...
		_ => panic!("Unknown client command, use 'gringron help client' for details"),
	}
	0
}
