...
/// GrinGron server commands processing
...
use gringron_p2p::msg::PeerAddrs;
use gringron_p2p::PeerAddr;
...
fn start_server_tui(
	...
	if config.run_tui.unwrap_or(false) {
		warn!("Starting GRINGRON in UI mode...");
...
/// stopping the GrinGron blockchain server. Processes all the command line
/// arguments to build a proper configuration and runs GrinGron with that
/// configuration.
pub fn server_command(
	...
	if let Some(a) = server_args {
		match a.subcommand() {
			...
			("", _) => {
				println!("Subcommand required, use 'gringron help server' for details");
			}
			(cmd, _) => {
				println!(":: {:?}", server_args);
				panic!(
					"Unknown server command '{}', use 'gringron help server' for details",
					cmd
				);
			}
		}
