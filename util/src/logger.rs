...
impl Default for LoggingConfig {
	fn default() -> LoggingConfig {
		LoggingConfig {
			...
			log_file_path: String::from("gringron.log"),
...
/// This filter is rejecting messages that doesn't start with "gringron"
/// in order to save log space for only GrinGron-related records
#[derive(Debug)]
struct GrinGronFilter;

impl Filter for GrinGronFilter {
	fn filter(&self, record: &Record<'_>) -> Response {
		if let Some(module_path) = record.module_path() {
			if module_path.starts_with("gringron") {
