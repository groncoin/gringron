...
This document describes smart contracts that can be setup using GrinGron even
though the GrinGron chain does not support scripting. All these contracts rely
...
document or the GrinGron development team. Most of the credit should be attributed
...
This section is meant as a reminder of some crucial features of the GrinGron chain.
...
built for a GrinGron transaction by taking the above private key `x` to be the sum
...
public key `r*G`, and allows to verify non-inflation for all GrinGron transactions
(and signs the fees).
...
receiving party. As GrinGron outputs are completely obscured by Pedersen
...
Alice wants to pay Bob in gringrons. She starts the transaction building process:
...
Alice has gringrons and Bob has bitcoin. They would like to swap. We assume Bob
...
ready to send her gringrons to Bob if he reveals `x`.

First, Alice sends her gringrons to a multiparty timelock contract with a refund
...
1. As soon as Bob broadcasts the final transaction to get his new gringrons, Alice
