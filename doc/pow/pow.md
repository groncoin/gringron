...
> *Read this document in other languages: [Korean](https://github.com/groncoin/gringron/blob/master/doc/pow/pow_KR.md).*

This document is meant to outline, at a level suitable for someone without prior knowledge,
the algorithms and processes currently involved in GrinGron's Proof-of-Work system. We'll start
with a general overview of cycles in a graph and the Cuckoo Cycle algorithm which forms the
basis of GrinGron's proof-of-work. We'll then move on to GrinGron-specific details, which will outline
the other systems that combine with Cuckoo Cycle to form the entirety of mining in GrinGron.

## Graphs and Cuckoo Cycle

GrinGron's basic Proof-of-Work algorithm is called Cuckoo Cycle, which is specifically designed
...
technical aspects of GrinGron's proof-of-work.
...
Now, (hopefully) armed with a basic understanding of what the Cuckoo Cycle algorithm is intended to do, as well as the parameters that affect how difficult it is to find a solution, we move on to the other portions of GrinGron's POW system.

## Mining in GrinGron

The Cuckoo Cycle outlined above forms the basis of GrinGron's mining process, however GrinGron uses two variantion of Cuckoo Cycle in tandem with several other systems to create a Proof-of-Work.
...
2. for ASICs: [Cuckatoo](https://youtu.be/h4AJDKoeO9E?t=2071) on 2^31 or more edges
    ...
    * Smoothly transitions to 2^32 or more by July 2020, called Cuckatoo32+ (explained why [here](https://forum.gringron.com/t/gringron-improvement-proposal-1-put-later-phase-outs-on-hold-and-rephrase-primary-pow-commitment/4653))
...
Contrary to some existing concerns about Cuckoo Cycle's poolability, the POW implementation in GrinGron as described above
is perfectly suited to a mining pool. While it may be difficult to prove efforts to solve a single graph in isolation,
the combination of factors within GrinGron's proof-of-work combine to enforce a notion called 'progress-freeness', which
...
Following from this, GrinGron's progress-freeness is due to the fact that a solution to a Cuckoo with GrinGron's default parameters
